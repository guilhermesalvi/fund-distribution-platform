...
Dispara quando o pedido é documentar o que já foi construído: "PRD do módulo X", "documente o que construímos". Neste modo o autor olha para código e telas, então a atração para descrever mecanismo em vez de comportamento é a máxima possível; por isso o capability test (writing.md, Capability test) tem peso extra.
...
- **Intenção inferida.** Toda intenção inferida é `[PREMISSA]`, e a marcação é feita sem economia, porque intenção engenheirada em reverso é frágil.
...
- **Deprecação.** Quando é relevante, deprecação entra em Não-objetivos ou em Trade-offs Declarados (writing.md, Seções), dizendo qual interface antiga não é coberta, o que não será migrado e que o cronograma não está comprometido.
