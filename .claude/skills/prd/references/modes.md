...
- **Deprecação.** Quando o PRD substitui interface já publicada, deprecação entra em Trade-offs Declarados quando alguma interface antiga deixa de ser coberta (há custo) e em Não-objetivos quando nada publicado deixa de funcionar (writing.md, Seções), dizendo qual interface antiga não é coberta, o que não será migrado e que o cronograma não está comprometido.
