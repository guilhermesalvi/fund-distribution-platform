...
| | |
|---|---|
| **Originating Context** | ReservationBook; affects Allocation |
...
- **Alteração e cancelamento livres até o fechamento (BOOK-10 a BOOK-12).** *Cost:* a demanda acumulada não é compromisso, o livro pode encolher antes do fechamento antecipado e a regra diverge da letra do art. 65, § 4º. *Reason:* a plataforma modela o livro da corretora, não o do coordenador; a irrevogabilidade que o Allocation exige é a do livro fechado. Vinculada à premissa em Open Questions.
...
- **Status como projeção do resultado do Allocation (BOOK-17).** *Cost:* o desfecho existe em dois contextos; após a revogação o Allocation mantém o resultado enquanto o livro mostra sem efeito. *Reason:* "o que aconteceu com a minha reserva" é pergunta do livro; a quantidade reservada nunca muda.
...
- **Sem eventos de saída na v1.** *Cost:* nenhum contrato para reagir às reservas em tempo real. *Reason:* evento sem consumidor é acoplamento implícito.
...
- **Given** um livro com reservas ativas e uma Withdrawn, **when** é fechado e consultado pelo Allocation, **then** somente as ativas no fechamento compõem a entrada; reservas aceitas no mesmo instante têm ordens distintas (BOOK-14 a BOOK-16).
- **Given** oferta Aberta com reservas ativas, **when** é revogada, **then** elas ficam Sem efeito (BOOK-18).
- **Given** resultado para reserva Withdrawn, **when** recebido, **then** é rejeitado e registrado, sem mudar o pedido (BOOK-19).
