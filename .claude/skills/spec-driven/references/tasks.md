...
| **Commit** | Mensagem planejada no formato que o repositório convenciona; pode ficar vazia quando commits não estão autorizados. Mensagem planejada, commit autorizado e commit executado são estados distintos (SKILL.md, Contrato, item 3) |
...
Rode `lint_tasks.py <tasks.md> --spec <spec.md>` (SKILL.md, Gates). Requisito da spec sem task é código que não vai existir.
