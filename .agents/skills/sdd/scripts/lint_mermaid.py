...
Extracao de fences (subconjunto do CommonMark, com uma excecao deliberada):
  ...
  - fences nao-mermaid sao rastreados apenas para que um bloco mermaid dentro
    de um fence externo demonstrativo (ex.: o template de um artefato inteiro
    dentro de ````markdown numa referencia desta skill) seja extraido; o fence
    externo de 4 crases nao e fechado por uma linha de 3 crases. Aqui o script
    diverge do CommonMark de proposito: pelo padrao, o conteudo do fence
    externo seria texto literal.
  - bloco ``` que nao e mermaid e ignorado.

O parse Mermaid NAO substitui a validacao da estrutura Markdown do documento
(secoes, IDs): isso e do lint_spec.py, do lint_design.py, do lint_tasks.py e
do lint_adr.py, que nao chamam este script. Este script so responde "o
diagrama renderiza?".
