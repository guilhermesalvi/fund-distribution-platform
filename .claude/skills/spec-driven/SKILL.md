...
- **Precedência.** Pedido do usuário na sessão > convenção do projeto (CLAUDE.md do repositório, rules, CONTRIBUTING, docs) > instruções pessoais do usuário (CLAUDE.md global) > defaults desta skill, para layout, formato de commit, idioma, forma de ADR e comandos. A skill preenche o que ninguém fixou e diz quando um default foi substituído.
- **Aprovações e autorizações** (fonte canônica; os demais lugares citam). Quatro atos distintos: (1) **aprovação de conteúdo**, o gate entre artefatos: cada artefato é revisado e aprovado pelo usuário antes do seguinte, e apresentar e começar o próximo no mesmo fôlego pula o gate; (2) **autorização para implementar** (editar arquivos da mudança na árvore de trabalho); (3) **autorização para commit** local; (4) **autorização para operação externa** (push, deploy, banco compartilhado, qualquer efeito remoto ou destrutivo), sempre por ação. Pedido explícito ("implementa a spec", "faz a T3") concede (2); "commita" concede (3); (4) nunca é implícita. Autorização concedida vale para a sessão ou a mudança e não é pedida de novo. Silêncio nunca é aprovação nem autorização. Aprovação refere-se à revisão aprovada: alteração material no artefato (comportamento, escopo, contrato) invalida a aprovação dele e dos dependentes que a citam; correção editorial não reabre nada (modes.md, Estados).
- **Decisão, premissa ou bloqueio.** Toda dúvida cai em uma de três: (a) decisão ordinária dentro da autonomia já concedida: decida, registre onde o artefato manda (assumption, decisão técnica, memória) e siga, sem nova confirmação; (b) `[PREMISSA]` explícita com default e racional, que permite avançar e fica revisável; (c) decisão material do usuário (escopo, regra de negócio, trade-off, operação externa): pergunte e bloqueie só o trabalho que depende dela, continuando o resto. Delegação explícita ("decide você") vira (a). Sem resposta, o item dependente fica bloqueado e registrado; nada é adotado "por default" e orçamento de perguntas (specify.md, Clarify) limita a rodada, não converte decisão material pendente em premissa. Vale em Specify, Design, Execute e Verify.
- **Falha é entregável.** Linter com HARD que você não consegue resolver, dependência indisponível, fonte ausente ou verificação bloqueada se apresentam como relatório de falha ou de validação incompleta, com o que falta; nunca como artefato aprovável, e nunca se esconde a falha para "não apresentar HARD".
...
- Válvula de segurança: Execute sem `tasks.md` começa listando passos inline; passou de 5, revelou dependência não trivial ou risco nomeado → pare, declare o tier novo e crie o artefato que faltou (execute.md, Passos inline).
...
Os scripts checam o que a auto-revisão da LLM faz mal: conformidade mecânica que sofre drift. O que cada um checa está no docstring (`python <skill-dir>/scripts/<nome>.py` sem argumentos; use `python3` onde `python` não existir). `HARD` → corrija e rode de novo; artefato com HARD pendente não é aprovável. HARD com prefixo `INCOMPLETO` é validação incompleta (fonte ausente, git ou parser indisponível): relate como tal, nunca como sucesso (Posicionamento, Falha é entregável). `WARN` → heurística; julgue. Linter verde é esqueleto conforme, não artefato bom; as passadas de julgamento (Revise) continuam obrigatórias. Sem ferramenta de execução de código, faça as mesmas checagens lendo o artefato e diga que foi manual.
...
| Antes de apresentar a spec | `lint_spec.py <spec.md> [--living <spec-viva.md>]` (IDs do PRD citados resolvem em `/docs/prd`; prefixo da spec distinto dos prefixos de PRD e igual ao dos IDs; PRD ausente é HARD `INCOMPLETO`; `prd-rev` divergente é HARD, ausente é WARN; `--print-prd-rev <prd.md>` imprime a revisão) |
| Antes de apresentar as tasks | `lint_tasks.py <tasks.md> --spec <spec.md> [--commit-max-len N] [--commit-no-scope]` (`--spec` obrigatório) |
| Em cada commit | `check_commit.py --message "<msg>" [--no-scope] [--max-len N]` (ou `--file` como hook `commit-msg`): confere a forma Conventional Commits; a regra mais estrita do repositório entra pelas opções |
...
Revisão é parte da entrega em toda entrada: a passada mecânica (Gates) e três de julgamento. Falha em qualquer uma → ajuste antes de apresentar.
...
- **Nova mudança:** roteamento → tier → Specify → [Design] → [Tasks] → Analyze (verify.md) → Execute → Verify → arquivar (memory.md), com o gate de aprovação entre cada artefato; o que cada modo dispensa e o substituto está em modes.md.
