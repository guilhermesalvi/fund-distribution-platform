...
5. **Formas por seção.** Declared Trade-offs com *Cost* e *Reason* em cada bullet, de até duas linhas; Success Metrics com guardrail; premissa "if false" só como primeiro bullet, em negrito, de Open Questions; Weakest Point seguido só de References; cenário Given/When/Then citando ID; bullet de Regulatory Considerations apontando ID, com nota de até 20 palavras; contexto listado em `; affects` com linha em Dependencies and Risks (writing.md, Seções).
...
8. **Diagramas.** Cada bloco Mermaid lido linha a linha: fence fechado, tipo de diagrama declarado, sintaxe que renderiza, nenhuma palavra reservada como alias, rótulo citando ID e, no `stateDiagram-v2`, a tabela com coluna Identifier ao lado (writing.md, Diagramas).
9. **Tags e placeholders.** Só `[ASSUMPTION]` e `[GAP]` entre colchetes; nenhum TBD, TODO ou `[nome]`; nenhum parágrafo de prosa repetido de outro PRD da pasta.
...
1. **Capability test.** A Proposed Solution, a frase de solução do Executive Summary e cada FR descrevem comportamento observável, não mecanismo (writing.md, Capability test); em reverse PRD o alcance é outro, e quem o fixa é uma fonte só (modes.md, Modo reverse PRD). Cada nome de mecanismo (tecnologia, componente, padrão de implementação) na Proposed Solution, na frase de solução do Executive Summary ou num FR é uma ocorrência; no reverse PRD, em qualquer seção. Termo que o pedido ou o material fixa como parte do produto, com uma linha de razão no chat, não é ocorrência.
2. **Uma regra, um lugar.** Cada regra existe em um único FR e o resto cita o ID. Paráfrase que diverge do FR não se resolve aqui: vira `[GAP]` em Open Questions (writing.md, Uma regra, um lugar).
3. **Tags.** Toda inferência está marcada `[ASSUMPTION]` ou `[GAP]`; discovery sintetizado tem a origem marcada (intake.md, Material de discovery).
4. **Forma.** Nenhuma seção existe só para cumprir forma (writing.md, Seções). O que é forma verificável já foi conferido na checagem de forma (workflow.md, Checar), inclusive a seção `##` fora da tabela; aqui sobram os dois que só a leitura de conteúdo pega:
   - bullet inserido para completar contagem, isto é, bullet cuja remoção não tira informação nenhuma do PRD; bullet que uma regra de writing.md exige, como o guardrail de Success Metrics marcado `[ASSUMPTION]` (writing.md, Seções), nunca é ocorrência;
   - Weakest Point sem decisão de julgamento sobre fatos conhecidos (corte de escopo, threshold, priorização ou usuário-alvo), isto é, cosmético.
5. **Idioma e redação.** A prosa segue o idioma fixado pela precedência (conventions.md, Idioma); há um conceito por parágrafo (writing.md, Redação). Ocorrência: parágrafo de prosa em idioma diferente do fixado, parágrafo com dois assuntos, ou dois parágrafos adjacentes sobre o mesmo ponto. Heading e rótulo já foram conferidos na checagem de forma e não contam aqui.
...
2. Aponte o Weakest Point, quando a seção existir.
3. Apresente todas as perguntas que efetivamente entraram em Open Questions (writing.md, Seções).
...
- Mudança pedida pelo usuário que toca duas ou mais seções, ou que altera Context and Problem ou Proposed Solution, regenera o PRD inteiro, porque a consistência entre seções é o que se perde no ajuste pontual. Mudança localizada (um FR, um threshold, uma frase, uma `[GAP]`) é ajuste pontual. Regeneração ou ajuste pedido pelo usuário reinicia o passo Checar.
