...
Task indicada pelo usuário ("faz a T3") ou a próxima disponível. Dependência não concluída → "T3 depende de T2, que não está feita. Faço T2 antes?"

### 2. Declarar o plano

Antes de escrever código, em 3 linhas: **arquivos** (só os que a task lista), **abordagem**, **como vou verificar**. Premissa ou incerteza → diga. Se enxerga abordagem mais simples que a do design, diga antes; se discorda do design, discorde — não seja sicofanta.

### 3. Testes derivados da spec

Quando a task tem `Tests` ≠ none:

- Contrato item 1: os testes vêm do "Pronto quando" e dos requisitos da spec. Cada requisito da task tem ≥1 assertion cujo valor afirmado é o **resultado que a spec define** (status, mensagem, estado, evento). Spec sem resultado preciso → registre como *lacuna de precisão da spec* e volte à spec; não escreva assertion vaga que passa em silêncio.
...
Rode o comando do nível de gate da task (`Comandos de Gate` em `tasks.md`, ou do plano inline); exit ≠ 0 → pare, corrija, rode de novo (contrato item 2). Gate que não pode ser executado (SDK ausente, dependência indisponível) não é gate verde: a task fica bloqueada com o motivo. Contagem de testes ≥ anterior é sinal auxiliar: diminuição só é legítima com requisito REMOVED e a razão registrada.
...
3. "Um sênior chamaria isso de complicado demais?" Sim → simplifique, rode o gate de novo.
...
Antes de fechar: marque a task concluída em `tasks.md` (ou no plano inline) e, quando há delta, atualize `Status` na Rastreabilidade da spec (`Implementing` → `Verified` só depois do Verifier). Com commit autorizado, essas atualizações entram **no mesmo commit** da task:
...
- Cada task: plano declarado → testes da spec falhando → implementação mínima → gate verde → tabela de evidência → fechamento, com commit quando autorizado (ciclo, passos 2–7).
